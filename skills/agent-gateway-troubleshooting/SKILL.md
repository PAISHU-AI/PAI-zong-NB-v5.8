---
name: agent-gateway-troubleshooting
description: "Troubleshoot agent runtime gateway service startup, restart, launchd/system service drift, and API server startup failures after updates or config changes."
---
...
Troubleshoot agent runtime gateway service startup, restart, launchd/system service drift, and API server startup failures after updates or config changes.

## Use When

- The user request matches `agent-gateway-troubleshooting`, `agent gateway troubleshooting`, `software-development` or the skill description.
- Troubleshoot agent runtime gateway service startup, restart, launchd/system service drift, and API server startup failures after updates or config changes.
